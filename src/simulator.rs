// src/simulator.rs
// Manages the collection of Boids, perform neighbor searches using KDTree
...
pub struct BoidSimulator {
    pub boids: Vec<Boid>,    // Vector holding all boids instances
    config: BoidsConfig,     // Simulation parameters for boids
    screen_dims: (f32, f32), // Screen width and height
    kdtree: KdTree<f32, usize, [f32; 2]>, // KDTree for effient neighbor search
                             // Stores boid indices (usize) associated with position ([f32,2])
}
...
impl BoidSimulator {
    ...
    /// # Arguments
    ///
    /// * `config` - The boidConfig containing simulation paramaters
    ...
    pub fn new(config: BoidsConfig, screen_dims: (f32, f32)) -> Self {
        BoidSimulator {
            ...
            config,
            screen_dims,
            // Initialize an empty KDTree with 2 dimension (x, y)
            kdtree: KdTree::new(2),
        }
    }
    /// Adds a new Boid to the simulation at a specific position.
    /// Uses the porvided random number generator for initial velocity
    ///
    /// # Arguments
    ///
    /// * 'pos' - The inital position Vec2 for the new boid
    ...
    fn build_kdtree(&mut self) {
        ...
        for (i, boid) in self.boids.iter().enumerate() {
            ...
            // Add the point and its corresponding index usize int eh tboid vector
    ...
    /// 1. Rebuilds the KDTree for efficeient neighbor finding
    /// 2. Calculate velocity changes for all boids based on neighbors.
    ...
    pub fn update(&mut self) {
        ...
        // 1. REbuld KDTree with current boid position
        self.build_kdtree();

        // 2. Calculate velocity changes for all boids
        // We store the changes temporarily to avoid modifying while iterating
        ...
        for i in 0..self.boids.len() {
            ...
            // Find neighbors within th evisible range using the KDTree
